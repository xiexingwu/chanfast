...
MPI_Comm world_comm, sm_comm, cart_comm;
int world_size, sm_size;
int sm_rank;
...
void initMpiPlan(){
  ...
  // MPI_Comm row_comm;
  // MPI_Comm col_comm;
  // MPI_Comm_split(MPI_COMM_WORLD, cart_coord[0], cart_coord[1], &row_comm);
  // MPI_Comm_split(MPI_COMM_WORLD, cart_coord[1], cart_coord[0], &col_comm);
...
void translateSmToCart()
{
  ...
  int *tmp = malloc(sm_size * 2 * sizeof(int));
